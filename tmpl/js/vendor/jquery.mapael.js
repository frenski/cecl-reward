...
(function($) {
			...
			*/
			$(this).bind('update', function(e, updateOptions, resetAreas, resetPlots, animDuration, easing) {
				...
				for (id in areas) {
					...
					$.fn.mapael.unSetTooltip(areas[id], $tooltip, paper);
					
					if (elemParams.tooltip && elemParams.tooltip.content) {
					  $.fn.mapael.setTooltip(areas[id].mapElem, $tooltip);
	...
	*/
	...
	$.fn.mapael.unSetTooltip = function(elem, $tooltip, paper) {
		$(elem.mapElem.node).unbind("mouseover").unbind("mouseout").unbind("mousemove");
		$.fn.mapael.setHover(paper, elem.mapElem, elem.textElem);
	};
