...
;(function($){
	...
	var PicCarousel = (function(){
				...
				if(dir === "left"){
					me.posterItems.each(function(){
						var self = $(this),
							...
							width = prev.width(),
							height = prev.height(),
							zIndex = parseInt(prev.css("zIndex")),
					...
					me.posterItems.each(function(){
						var self = $(this),
							...
							width = next.width(),
							height = next.height(),
							zIndex = parseInt(next.css("zIndex")),
        ...
        let maxId = zIndexArr.reduce((iMax, x, i, arr) => x > arr[iMax] ? i : iMax, 0);

        // console.log(zIndexArr, maxId);

        if(this.settings.onInteractEnd!=''){
          this.settings.onInteractEnd(maxId);
        }
