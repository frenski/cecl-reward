...
$(document).ready( function(){
	...
	var changeData = function (id) {
		...
		for (var i=0; i<tParams.infData[id].data.length; i++) {
			...
			// console.log($('#infg-canvas-wrapper'+i));
			$('#infg-canvas-wrapper'+i)
				.css("background-image","url('../files/"+tParams.infItems[i].imgs[id]+"')");
			$('#infg-label-text').html($('#content_text10'+id).html());
		}
	}
	...
	$('.slider-button').click(function(){
		...
		if (id!=selected) {
			$("#slider-button" + selected).removeClass('button-selected');
			$("#slider-button" + id).addClass('button-selected')
															.removeClass("anim-pulsate");
			changeData (id);
			selected = id;
	...
	yLearnTmpl.loadPage(function(){
		$("#slider-button0").addClass('button-selected');
